...
AudioProcessingModule::AudioProcessingModule(int aec_type, bool enable_ns, int agc_type, bool enable_vad, int sample_rate, int sample_channels)
{
    system_delay = 0;
    ...
    if (1 == aec_type) {
        ...
        ap->echo_cancellation()->enable_metrics(true);              // added ByPark
        ap->echo_cancellation()->enable_delay_logging(true);        // added ByPark
    } else if (3 == aec_type) {
    ...
    if (enable_ns) {
        ...
        ap->noise_suppression()->set_level(static_cast<NoiseSuppression::Level>(3));
    }
    ...
    nearend_config = new StreamConfig(sample_rate, sample_channels, false);
    nearend_filtered_config = new StreamConfig(sample_rate, sample_channels, false);
    ...
    // farend_config = new StreamConfig(sample_rate, default_channels, false);
    nearend_fbuf = new float[num_frames * sample_channels];
    nearend_ibuf = new int16_t[num_frames * sample_channels];
    nearend_cbuf = new ChannelBuffer<float>(num_frames, sample_channels);
    nearend_filtered_cbuf = new ChannelBuffer<float>(num_frames, sample_channels);

    farend_config = new StreamConfig(sample_rate, sample_channels, false);
    ...
    farend_fbuf = new float[num_frames * sample_channels];
    farend_cbuf = new ChannelBuffer<float>(num_frames, sample_channels);
}
